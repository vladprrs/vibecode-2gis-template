...
test.describe('Header Layout Consistency', () => {
  ...
  test('Header elements have consistent dimensions across screens', async ({ page }) => {
    ...
    expect(dashboardHeaderDimensions.dragHandle.width).toBe(
      suggestHeaderDimensions.dragHandle.width
    );
    expect(dashboardHeaderDimensions.dragHandle.height).toBe(
      suggestHeaderDimensions.dragHandle.height
    );
  });
